...
test("refreshes resources into reactive getters", async () => {
  ...
  const calls = createFetch((url) => {
    ...
    if (url.pathname === "/api/agent")
      return json({
        location,
        data: [{ id: "work", request: { headers: {}, body: {} }, mode: "primary", hidden: false, permissions: [] }],
      })
    return undefined
  }, events)
  ...
  try {
    ...
    expect(data.location.agent.list(location)?.map((agent) => agent.id)).toEqual(["work"])
  } finally {
...
test("settles pending tools when a live failure arrives", async () => {
  ...
  try {
    ...
    emitEvent(events, {
      ...
      properties: { sessionID: "session-1", messageID: "msg_agent_1", timestamp: 0, agent: "work" },
    })
    emitEvent(events, {
      id: "evt_model_1",
