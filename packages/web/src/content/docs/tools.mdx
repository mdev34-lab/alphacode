...
custom tools and all MCP tools — is **deferred**: kept out of the request until the model looks it
up with one of two discovery tools.
...
Both tools accept an optional `category` (`all`, `builtin` or `mcp`). Tools returned by a search
become available on the model's next step and stay available for the rest of the session.
