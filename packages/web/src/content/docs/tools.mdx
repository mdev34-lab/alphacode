...
longer than 200 characters, backreferences, and quantified groups that contain a quantifier or an
alternation (`(a+)+`, `(a|a)*`). Those are the shapes that cause catastrophic backtracking, and a
running regex cannot be interrupted in JavaScript.
...
Search is BM25 over tool names and descriptions, with Unicode-aware tokenization and regular
plural folding, so `find issues` also matches a tool described as "list the issue tracker".
