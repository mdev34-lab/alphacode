...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"work"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenCode will fall back to `"work"` with a warning. The legacy id `"build"` is still accepted and resolves to `"work"`.
...
OpenCode injects a short STE-lite reply style into agent system prompts. It keeps chat terse without shrinking deliverables such as source, tests, docs, plans, or reviews.

Enabled by default. Set `ste_lite` to `false` to turn it off.
