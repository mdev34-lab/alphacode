...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const contextPressure = yield* SessionContextPressure.Service
    ...
    const loadSystemContext = (sessionID: SessionSchema.ID, agent: AgentV2.Selection) =>
      Effect.all(
        [systemContext.load(), skillGuidance.load(agent), referenceGuidance.load(), contextPressure.load(sessionID, agent)],
        {
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      // The same report feeds the agent-facing pressure nudge on the next turn.
      const published = transmission.recovered ? reduction.fallbackReport : reduction.report
      yield* events.publish(SessionEvent.Context.Prepared, {
        sessionID: session.id,
        timestamp: yield* DateTime.now,
        ...published,
      })
      yield* contextPressure.record(session.id, published)
