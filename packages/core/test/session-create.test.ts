...
describe("SessionV2.create", () => {
    ...
    Effect.gen(function* () {
      ...
      expect(
        yield* session.create({
          ...
          agent: AgentV2.ID.make("work"),
          model,
        }),
      ).toMatchObject({ location: { directory: location.directory, workspaceID }, agent: "work", model })
    }),
  )
  ...
  it.effect("returns the existing Session when one ID is reused with different create arguments", () =>
    Effect.gen(function* () {
      ...
      const changed = [
        ...
        { id, location, agent: AgentV2.ID.make("work") },
        {
          id,
          location,
  ...
  it.effect("returns the current Session projection after updates", () =>
    Effect.gen(function* () {
      ...
      yield* db.update(SessionTable).set({ agent: "work" }).where(eq(SessionTable.id, id)).run().pipe(Effect.orDie)

      expect(yield* session.create(input)).toMatchObject({ id: created.id, agent: "work" })
    }),
  )
  ...
  it.effect("returns the current Session projection after projected updates", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.create(input)).toMatchObject({ id, agent: "work" })
    }),
  )
