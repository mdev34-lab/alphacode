...
| Per-turn request assembly  | Steering, plan/work-switch, and final-step reminders                    | missing  | Add only reminders whose behavior remains part of V2.                                                                                  |
...
| Prompt/reference expansion | File, directory, media, and MCP-resource materialization                 | partial  | Materialize and normalize sources instead of lowering unresolved attachment metadata. Prompt attachments (pasted `data:`, mentioned `file://`, and remote `http(s)://` URLs) are now materialized to a managed store and lowered correctly (media bytes or inlined text) via `AttachmentStore`; MCP-resource materialization remains deferred. |
...
When a Session omits `agent`, both execution and permission evaluation use the default `work` agent. A caller must not observe `work` model behavior while permission checks silently evaluate an empty no-agent policy.
